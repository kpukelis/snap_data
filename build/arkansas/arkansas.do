...
forvalues month = `month_start'(1)`month_end' {
	...
	// import data 
	import excel "${dir_data}/csvs/`year'/COOR`monthname'`year'`suffix'.xlsx", allstring case(lower) clear
	dropmiss, force
...
*foreach num in 2 {
		...
		save "${dir_data}/_`year'_`month'_`num'.dta", replace
...
forvalues num = 1(1)2 {
	forvalues year = `year_start'(1)`year_end' {
		...
		forvalues month = `month_start'(1)`month_end' {
			...
			use "${dir_data}/_`year'_`month'_`num'.dta", clear
...
*				append using "${dir_data}/_`year'_`month'_`num'.dta"
			}
		}
	}
	tempfile data_`num'
...
// save 
save "${dir_data}/arkansas.dta", replace 
check 
