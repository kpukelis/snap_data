...
forvalues ym = `ym_start'(1)`ym_end' {
	...
	*qui sum obsnum if strpos(v1,"NW REGION") & strpos(v2,"ANDREW")
	...
	else if inrange(`ym',ym(2021,10),ym(2022,3)) {
...
*	local batch_start_5 = `r(min)'
	...
	if inrange(`ym',ym(2021,10),ym(2022,3)) {
	...
	if inrange(`ym',ym(2021,10),ym(2022,4)) {
		assert r(N) == 2
...
*		preserve
		...
		if `n' == 1 {
			...
			if inlist(`ym',ym(2015,4),ym(2015,5),ym(2017,11),ym(2017,12)) | inrange(`ym',ym(2019,7),ym(2019,10)) | inrange(`ym',ym(2021,10),ym(2022,4)) {
	...
	*forvalues n = 1(1)`num_pages' {
		if `ym' == `ym_start' {
			...
			*append using `_`ym'_page`n''
			...
			assert dup == 0
			drop dup 
			merge 1:1 ym using `_`ym'_page`n'', update nogen
		}	
	*}
}
